...
void main(void) {
    ...
    vec2 st = vec2(0.55 * cos(time * TAU), -0.05 * abs(sin(time * TAU)));

    vec4 bg = smoothstep(0.22, 0.21, length(uv - st)) * bgColor;
    vec4 sp =  smoothstep(0.2, 0.19, length(uv - st)) * spiral(time, (uv - st) / 0.2) * fgColor;
    vec4 line = smoothstep(0.02, 0.01, distance_to_line_segment(uv, st, vec2(0.0, 1.0))) * fgColor;
