...
import type { FlashTextAlign, FlashTextStyle, TextStyle } from "src/effects/webgl/Text"
...
export const ColorEditor = ({ label, value, onChange }: ColorEditorProps) => {
	...
	return (
		<>
			<Box component="div" style={{ width: "100%", background: toCssStringRGB(...value), padding: "0.25em", margin: "0 0.25em" }}>
		...
		<Grid container spacing={1}>
			...
			<Grid item xs={12} sm={6}>
				<TextField
					...
					value={value[0] ?? 0} onChange={v => onChange([parseFloat(v.target.value), value[1]])} />
			</Grid>
			<Grid item xs={12} sm={6}>
				<TextField
					type="number" label="Y" fullWidth inputProps={{ step: 0.01 }}
					value={value[1] ?? 0} onChange={v => onChange([value[0], parseFloat(v.target.value)])} />
			</Grid>
		</Grid>
	)
}
	...
	return (
		<Accordion expanded={open} onChange={(_, open) => setOpen(open)}>
			...
			<AccordionDetails>
				<Grid container spacing={3}>
					<Grid item xs={6}>
						<TextField inputProps={{ min: 0, step: 0.1 }} type="number" label="Font size" fullWidth value={value.size ?? 1} onChange={v => onChange({ ...value, size: parseFloat(v.target.value) })} />
					</Grid>
					<Grid item xs={6}>
						<TextField inputProps={{ min: 0, step: 1 }} type="number" label="Line width" fullWidth value={value.lineWidth ?? 1} onChange={v => onChange({ ...value, lineWidth: parseFloat(v.target.value) })} />
					</Grid>
					<Grid item xs={6}>
			...
			return (
				<Accordion expanded={open} onChange={(_, open) => setOpen(open)}>
					...
					<AccordionDetails>
						<Grid container spacing={3}>
							<Grid item xs={12}>
								<TextField inputProps={{ step: 1 }} fullWidth type="number" label="By" value={value.by} onChange={e => onChange({ ...value, by: parseInt(e.target.value, 0) })} />
							</Grid>
							<Grid item xs={12}>
								<PatternEditor value={value.children} onChange={children => onChange({ ...value, children })} />
							</Grid>
			...
			return (
				<Accordion expanded={open} onChange={(_, open) => setOpen(open)}>
					...
					<AccordionDetails>
						<Grid container spacing={3}>
							<Grid item xs={12}>
								<TextField fullWidth label="Shader" value={value.pattern} onChange={pattern => onChange({ ...value, pattern: pattern.target.value })} />
							</Grid>
							<Grid item xs={12}>
								...
								<ColorEditor label="Dim" value={value.colors.dim ?? [1, 1, 1, 1]} onChange={dim => onChange({ ...value, colors: { ...value.colors, dim } })} />
								<ColorEditor label="Pulse" value={value.colors.pulse ?? [1, 1, 1, 1]} onChange={pulse => onChange({ ...value, colors: { ...value.colors, pulse } })} />
								<ColorEditor label="Extra" value={value.colors.extra ?? [1, 1, 1, 1]} onChange={extra => onChange({ ...value, colors: { ...value.colors, extra } })} />
							</Grid>
						</Grid>
					</AccordionDetails>
				</Accordion>
